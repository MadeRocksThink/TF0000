...
const CLIENT_ID = "abcdefghijklmnopabcdefghijklmnop";
...
export function resolveNativeHostPath(
  ...
  if (configuredPath.trim()) {
    if (!isAbsolute(configuredPath) || !existsSync(configuredPath)) {
      throw new Error("tf0000.nativeHostPath must be an existing absolute file path");
    }
    return configuredPath;
  }
  const executable = process.platform === "win32" ? "tf0000-native-host.exe" : "tf0000-native-host";
  const candidates = [
    process.env.TF0000_NATIVE_HOST,
    join(extensionPath, "bin", executable),
    resolve(extensionPath, "..", "..", "target", "release", executable),
